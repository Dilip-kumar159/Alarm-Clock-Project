...
let incompleteAlarmsholder = document.getElementById("pending-alarms");

var alarmMusic = new Audio('alarmSound.mp3');
...
function realTime() {
    ...
    document.getElementById("real").innerHTML = hr + ":" + min + ":" + sec + AMPM;
}
setInterval(realTime, 1000);


// New Alarm Time in form of list
...
// alarm time button which triggers creating the alarm time element:
document.getElementById("set-button").addEventListener("click", function () {
    ...
    setInterval(() => {
        ...
        // When real time matches with alarm time, the alarm shows an alert,
        // after clicking okay button then music starts to play.
        if (arr.length != 0 && arr[0][0] == hr && arr[0][1] == min && arr[0][2] == sec && arr[0][3] == selectedAMPM) {
            alert("Alarm is started ringing");
            alarmMusic.play();
        }
    }, 1000);
})
